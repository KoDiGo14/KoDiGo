...
from resources import streamplay ###THANKS TO jsergio123!!!
###THANKS TO DANDY to use his addon as template and making this addon that makes happy my wife!!!
...
def Main_menu():
	...
	#addDir('[B][COLOR white]Trending Series[/COLOR][/B]',BASEURL,4,ART + 'trending.jpg',FANART,'')
	#addDir('[B][COLOR white]Browse All Series[/COLOR][/B]',BASEURL,6,ART + 'browse.jpg',FANART,'')
	addDir('[B][COLOR white]Navegar por letras[/COLOR][/B]',BASEURL ,9,ART + 'genres.jpg',FANART,'')
	#addDir('[B][COLOR white]Buscador[/COLOR][/B]','url',20,ART + 'search.jpg',FANART,'')
...
def Get_links(name,url): #10
	...
	for url in Regex:
		name = re.compile('http[s]?://(.+?)\.',re.DOTALL).findall(url)
		if 'sebuscar' in name:
			name.remove('sebuscar')
		for name in name:
...
def resolve(name,url,iconimage,description):
	#OPEN = client.request(url)
	#host = re.compile('<div class="wb-main">.+?<a rel="nofollow" target="_blank" href="(.+?)"',re.DOTALL).findall(OPEN)[0]
	host = url
